...
/**
 * @file Generating Swift for procedure blocks.
 */
...
export function procedures_defreturn(block: Block, generator: SwiftGenerator) {
  ...
  let code = 'func ' + funcName + '(' + args.join(', ') + ') -> Any {\n' + xfix1 + loopTrap + branch + xfix2 + returnValue + '}\n';
...
export function procedures_callreturn(block: Block, generator: SwiftGenerator): [string, Order] {
...
export function procedures_callnoreturn(block: Block, generator: SwiftGenerator) {
  // Call a procedure without a return value.
  const funcName = generator.getProcedureName(block.getFieldValue('NAME'));
  const args = [];
  const variables = block.getVars();
  for (let i = 0; i < variables.length; i++) {
...
export function procedures_ifreturn(block: Block, generator: SwiftGenerator) {
